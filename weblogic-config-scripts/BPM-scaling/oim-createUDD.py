        ...
        if not token == 'AdminServer' and not token == '':
            ...
            if not clusterNameStr.find(searchClusterStr) == -1:
                ...
                if track == 'oim':
                    ...
                    jdbcStoreName = 'OIMJMSServerJDBCStore_auto_'+str(serverCnt)
                    prefix = 'OIMJMS_'+str(serverCnt)
                elif track == 'jrf':
                    jmsServerName = 'JRFWSAsyncJMSServer_auto_'+str(serverCnt)
                    #fileStoreName = 'JRFWSAsyncJMSServerFileStore_auto_'+str(serverCnt)
                    jdbcStoreName = 'JRFWSAsyncJMSServerJDBCStore_auto_'+str(serverCnt)
                    prefix = 'JRFWSAsyncJMS_'+str(serverCnt)

                #createFileStore(fileStoreName, token)
                #print "Created File Store :- ", fileStoreName

                createJDBCStore(jdbcStoreName, prefix, token)
...
if ofm_version > 1115:
    ...
    print "Warning: JRF-WS components not created. This is expected behaviour for versions of OFM 11.1.1.5 and below."
