...
clear
echo "###############################################################################################"
echo "# This script gets the list of ASG's for CERT. Then depending on what the user wants either   #"
...
get_asg_list=$(aws autoscaling describe-auto-scaling-groups --region eu-west-1 | grep "AutoScalingGroupName" | grep evise-fpu-cert- | awk '{print $2; }' | sed 's/"//g' | sed 's/,/ /g');
...
while true; do
    read -p "This script will $1 the CERT ASG's. OK to continue? y/n" yn
    case $yn in
        [Yy]* ) action; break;;
        [Nn]* ) exit;;
