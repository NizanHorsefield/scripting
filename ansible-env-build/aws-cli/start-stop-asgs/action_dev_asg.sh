...
get_asg_list=$(aws autoscaling describe-auto-scaling-groups --region eu-west-1 | grep "AutoScalingGroupName" | grep evise-fpu-dev- | awk '{print $2; }' | sed 's/"//g' | sed 's/,/ /g');
...
while true; do
    read -p "This script will $1 the DEV ASG's. OK to continue? y/n" yn
    case $yn in
        [Yy]* ) action; break;;
        [Nn]* ) exit;;
