#!/bin/bash
########################################################################################################################
# This script will copy a given FPUExec Jar file from S3 to NFS, update the TAGS in the  ASG, then terminate   #
...
FPUEXEC_VERSION=$1
FPUEXEC_JAR=$2
FPUEXEC_SRC=s3://evise-fpu-deployment/common/exec/snapshots/com/elsevier/evise/fpu-exec/${FPUEXEC_VERSION}/${FPUEXEC_JAR}
FPUEXEC_DEST=/home/jenkins/fpu/exec
#FPUEXEC_DEST=/els/fpu/exec
...
printf "\n"
aws s3 cp ${FPUEXEC_SRC} ${FPUEXEC_DEST}/${FPUEXEC_JAR}
